<?php
/* Template Name: nursing-jobs-by-state */
get_header();
?>
...
$sql = "SELECT * FROM api_states WHERE 1 ";

if ($query != '') $sql .= " AND state_name LIKE :name";
...
$sql = 'SELECT COUNT(id) as count FROM api_states WHERE 1';
if ($query != '') $sql .= " AND state_name LIKE :name";
$stmt = $pdo->prepare($sql);
if ($query != '') $stmt->bindParam(':name', $query, PDO::PARAM_STR);
$stmt->execute();
...
<div class='jobs-page'>
  <div class='first'>
    ...
    <div class='container'>
      ...
      <form class='global-search-form' autocomplete='off'>
        <input type='text' name='query' placeholder='State...' />
  ...
  <div class='second container'>
    ...
    <div class='list'>
      ...
      <div class='item H5'>
        <div class='content'><?php echo $state['state_name']; ?></div>
        <a class='link' href='<?php echo SEARCH_JOBS_LINK; ?>/search-jobs-in-<?php echo $state['state_link']; ?>'>
          <span>View jobs</span>
